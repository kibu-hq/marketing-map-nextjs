...
export default function StateInfoPanel({ isOpen, onClose, stateInfo, contentData }: StateInfoPanelProps) {
  ...
  const accountExecutive = stateInfo ? getAccountExecutiveForState(stateInfo.name, teamData) : null;

  useEffect(() => {
    if (isOpen && accountExecutive && (window as any).Calendly) {
      const preloadContainer = document.getElementById('calendly-preload-container');
      if (preloadContainer) {
        preloadContainer.innerHTML = '';
        (window as any).Calendly.initInlineWidget({
          url: `${accountExecutive.demo_link}?hide_gdpr_banner=1`,
          parentElement: preloadContainer,
        });
      }
    }
  }, [isOpen, accountExecutive]);
  ...
  return (
    <>
      <div id="calendly-preload-container" style={{ display: 'none' }}></div>
